...
<div class="subheader">
	<h1 class="subheader-title">
		<i class='subheader-icon fal fa-exclamation-circle'></i>Enfermedades
		<small>
...
<div class="row">
	<div class="col-xl-12">
        <div id="panel-6" class="panel d-none">
			...
			<div class="panel-container show">
				<div class="panel-content p-0">
					<form id="editar_enfermedad">
                        <input type="hidden" name="metodo" value="Actualizar" id="metodo">
                        <input type="hidden" name="id_enfermedad" value="0" id="id_enfermedad">
                        <div class="panel-content">
						<div class="form-row">
                            <div class="col-md-6 col-sm-12">
                                <label for="nombre_paciente">Agrega Enfermedad</label>
                                <input type="text" list="enfermedades" class="form-control" id="nombre_enfermedad" name="nombre_enfermedad" required>
		...
		<div id="panel-1" class="panel">
			<div class="panel-hdr">
				<h2>
					Enfermedades<span class="fw-300"><i>Lista</i></span>
				</h2>
				<div class="panel-toolbar">
...
<script>
	$(document).ready(function()
	{
                ...
                $("#menu_enfermedad").addClass('active');


				$("#editar_enfermedad").submit(function(e){
					e.preventDefault();
					if($("#metodo").val() == "Guardar")
					{
                        modal = cargando("Guardando Enfermedad", "Guardando...");
						var check = "true";
					}
					else{
						if($("#metodo").val()=="Actualizar"){
                            modal = cargando("Editando Enfermedad", "Editando...");
							var check = "true";
						}
						else{
							var check = "false";
					...
					if(check == "true"){
						var formdata = new FormData(document.getElementById("editar_enfermedad"));
                        formdata.append('_token',$('meta[name="csrf-token"]').attr('content'));
                        formdata.append('id_paciente',$('#id_paciente').val());
                        formdata.append('id_enfermedad',$('#fecha_enfermedad').val());
						if($("#metodo").val()=="Actualizar"){
							formdata.append('_method','PUT');
                            formdata.append('id_paciente', $("#id_paciente").val());
                            formdata.append('id_enfermedad', $("#id_enfermedad").val());
						}
						else{
						...
						if(data.estado=="true"){
							...
							var rowToScrollTo = document.getElementById('editar_enfermedad');
	...
	function nuevo(){
        ...
        $("#comentario").val("");
        document.getElementById("nombre_enfermedad").disabled = false;
		$("#panel-6").removeClass('d-none');
	}
	function editar(id){
		$("#id_enfermedad").val(id);
        modal = cargando("Cargando enfermedad", "Cargando...");
		$("#metodo").val('Actualizar');
		$.ajax({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
			url: 'enfermedad/'+id+'/paciente/'+$("#id_paciente").val(),
            type: 'GET',
			processData: true,
			contentType: false,
			success: function(data){
			...
			if(data.estado=="true"){
				...
				$("#nombre_enfermedad").val(data.enfermedad.enfermedad['nombre_enfermedad']);
                $("#comentario").val(data.enfermedad['comentario']);
                document.getElementById("nombre_enfermedad").disabled = true;
				$("#panel-6").removeClass('d-none');
	...
	function eliminar(id){
		...
		.then(function(result) {
			if (result.value) {
                modal = cargando("Eliminando enfermedad", "Eliminando...");
